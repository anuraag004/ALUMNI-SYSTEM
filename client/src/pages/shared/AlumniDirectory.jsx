...
const AlumniCard = ({ alumni, onContact, onMentor }) => (
    <div className="glass-card-interactive p-6 card-shine group">
                ...
                <div className="flex items-start justify-between gap-2">
                    <div>
                        <h3 className="font-bold text-white font-display group-hover:text-brand-300
                                      transition-colors duration-300">{alumni.displayName}</h3>
                        <p className="text-sm text-slate-400">{alumni.currentRole || 'Alumni'}</p>
                        {alumni.currentCompany && (
                            <p className="text-sm text-brand-400 font-medium">@ {alumni.currentCompany}</p>
                        )}
                    </div>
                ...
                {alumni.skills?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-3">
                        ...
                        {alumni.skills.length > 4 && (
                            <span className="badge bg-surface-elevated/60 text-slate-400
                                           border border-surface-border/40">+{alumni.skills.length - 4}</span>
                        )}
                    </div>
                )}
    ...
    return (
        <DashboardLayout>
            <div className="page-container">
                <h1 className="section-heading animate-slide-up">Alumni Directory</h1>

                {/* Filters */}
                <form onSubmit={handleSearch}
                      className="glass-card p-5 flex flex-col sm:flex-row gap-3 animate-slide-up"
                      style={{ animationDelay: '60ms' }}>
                    <input value={search} onChange={(e) => setSearch(e.target.value)}
                        className="input flex-1" placeholder="Search by name, company, role…" />
                    <select value={dept}
                            onChange={(e) => { setDept(e.target.value); setPage(1); fetch(search, e.target.value, 1) }}
                        ...
                        <>
                            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 stagger-children">
                                {alumni.length === 0
                                    ...
                                    : alumni.map((a) => (
                                        <AlumniCard key={a.uid} alumni={a}
                                                   onContact={handleContact}
                                                   onMentor={user?.role === 'student' ? handleMentor : undefined} />
                                      ))
                                }
                            </div>
                            ...
                            {total > LIMIT && (
                                <div className="flex justify-center items-center gap-3 mt-6">
                                    <button disabled={page === 1}
                                            onClick={() => { setPage(page - 1); fetch(search, dept, page - 1) }}
                                            className="btn-ghost disabled:opacity-30">← Prev</button>
                                    <span className="px-4 py-2 text-sm text-slate-400 font-medium">
                                        Page {page} of {Math.ceil(total / LIMIT)}
                                    </span>
                                    <button disabled={page >= Math.ceil(total / LIMIT)}
                                            onClick={() => { setPage(page + 1); fetch(search, dept, page + 1) }}
