// src/pages/auth/Login.jsx
import { useState } from 'react'
import { Link, useNavigate, useLocation } from 'react-router-dom'
import { useAuth } from '../../context/AuthContext'

const Login = () => {
    const { login, loginWithGoogle } = useAuth()
    const navigate = useNavigate()
    const location = useLocation()
    const from = location.state?.from?.pathname || null

    const [form, setForm] = useState({ email: '', password: '' })
    const [loading, setLoading] = useState(false)
    const [error, setError] = useState('')

    const redirect = (user) => {
        if (from) return navigate(from, { replace: true })
        if (user.role === 'admin') return navigate('/admin')
        if (user.role === 'alumni') return navigate('/alumni-dashboard')
        navigate('/student')
    }

    const handleSubmit = async (e) => {
        e.preventDefault()
        setError('')
        setLoading(true)
        try {
            const user = await login(form)
            redirect(user)
        } catch (err) {
            setError(err.message || 'Login failed. Please check your credentials.')
        } finally { setLoading(false) }
    }

    const handleGoogle = async () => {
        setError('')
        setLoading(true)
        try {
            const user = await loginWithGoogle()
            redirect(user)
        } catch (err) {
            setError(err.message || 'Google sign-in failed.')
        } finally { setLoading(false) }
    }

    return (
        <div className="min-h-screen bg-surface flex">
            {/* ── Left decorative panel (md+) ────────────────────── */}
            <div className="hidden md:flex md:w-1/2 lg:w-[55%] relative overflow-hidden
                           bg-gradient-to-br from-brand-900/80 via-surface to-surface-card items-center justify-center">
                {/* Floating blobs */}
                <div className="floating-shape top-1/4 left-1/4 h-72 w-72 bg-brand-500/25"
                     style={{ animationDelay: '0s' }} />
                <div className="floating-shape bottom-1/3 right-1/4 h-56 w-56 bg-purple-500/20"
                     style={{ animationDelay: '3s' }} />
                <div className="floating-shape top-1/2 left-1/2 h-40 w-40 bg-brand-400/15"
                     style={{ animationDelay: '1.5s' }} />

                <div className="relative z-10 max-w-md px-12 animate-slide-up">
                    <div className="h-14 w-14 rounded-2xl bg-gradient-brand flex items-center justify-center
                                   text-white text-xl font-bold shadow-glow-brand-lg mb-8">AC</div>
                    <h2 className="text-4xl font-bold text-white font-display leading-tight mb-4">
                        Welcome back to<br />
                        <span className="gradient-text">AlumniConnect</span>
                    </h2>
                    <p className="text-slate-400 text-lg leading-relaxed">
                        Your AI-powered bridge between students and alumni. Sign in to access mentorship,
                        jobs, and career tools.
                    </p>
                    {/* Feature pills */}
                    <div className="flex flex-wrap gap-2 mt-8">
                        {['AI Matching', 'Real-time Chat', 'Resume Analysis', 'Job Board'].map((f) => (
                            <span key={f} className="badge-indigo text-xs">{f}</span>
                        ))}
                    </div>
                </div>
            </div>

            {/* ── Right form panel ───────────────────────────────── */}
            <div className="flex-1 flex items-center justify-center px-6 py-12 relative">
                {/* BG orbs for mobile */}
                <div className="pointer-events-none absolute inset-0 overflow-hidden md:hidden">
                    <div className="floating-shape top-0 left-1/4 h-72 w-72 bg-brand-500/10" />
                    <div className="floating-shape bottom-0 right-1/4 h-56 w-56 bg-purple-500/10"
                         style={{ animationDelay: '2s' }} />
                </div>

                <div className="relative w-full max-w-md animate-slide-up">
                    {/* Mobile-only logo */}
                    <div className="text-center mb-8 md:hidden">
                        <Link to="/" className="inline-flex items-center gap-2 font-bold text-xl">
                            <span className="h-10 w-10 rounded-xl bg-gradient-brand flex items-center
                                           justify-center text-white shadow-glow-brand">AC</span>
                        </Link>
                    </div>

                    <div className="mb-8">
                        <h1 className="text-3xl font-bold text-white font-display">Welcome back</h1>
                        <p className="text-slate-400 mt-2">Sign in to your account</p>
                    </div>

                    <div className="glass-card p-8 shadow-card-elevated">
                        {error && (
                            <div className="mb-5 flex items-center gap-2 rounded-xl bg-rose-500/10
                                          border border-rose-500/20 px-4 py-3 text-sm text-rose-400
                                          animate-slide-down">
                                <span>⚠️</span> {error}
                            </div>
                        )}

                        {/* Google button */}
                        <button onClick={handleGoogle} disabled={loading}
                            className="btn-ghost w-full mb-5 border-surface-border/60
                                       hover:shadow-glow-brand/30">
                            <svg className="h-5 w-5" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" /><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" /><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" /><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" /></svg>
                            Continue with Google
                        </button>

                        <div className="flex items-center gap-3 mb-5">
                            <hr className="flex-1 border-surface-border/50" />
                            <span className="text-xs text-slate-500 font-medium">or email</span>
                            <hr className="flex-1 border-surface-border/50" />
                        </div>

                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-300 mb-1.5">Email</label>
                                <input type="email" required value={form.email}
                                    onChange={(e) => setForm({ ...form, email: e.target.value })}
                                    className="input" placeholder="you@example.com" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-300 mb-1.5">Password</label>
                                <input type="password" required value={form.password}
                                    onChange={(e) => setForm({ ...form, password: e.target.value })}
                                    className="input" placeholder="••••••••" />
                            </div>
                            <button type="submit" disabled={loading}
                                    className="btn-primary w-full py-3 mt-2">
                                {loading ? (
                                    <span className="flex items-center gap-2">
                                        <span className="h-4 w-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                                        Signing in…
                                    </span>
                                ) : 'Sign In'}
                            </button>
                        </form>

                        <p className="mt-6 text-center text-sm text-slate-400">
                            Don't have an account?{' '}
                            <Link to="/register" className="text-brand-400 hover:text-brand-300
                                                           font-semibold transition-colors duration-200">
                                Create one
                            </Link>
                        </p>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Login
